/**
 * 邮箱用户专用页面逻辑
 * 支持 admin 和 mailbox 两种角色
 */

// 全局状态
let currentUser = null;
let currentMailbox = null;
let userMailboxes = []; // 管理员的邮箱列表
let emails = [];
let currentPage = 1;
const pageSize = 20;
...
function initializeElements() {
  elements = {
    ...
    // 管理员专用元素
    mailboxSelector: document.getElementById('mailbox-selector'),
...
async function initializeAuth() {
  try {
    ...
    // 支持 admin 和 mailbox 两种角色
    if (data.role !== 'mailbox' && data.role !== 'admin') {
      redirectToLogin('无权访问此页面');
      return;
    }
    ...
    if (data.role === 'admin') {
      ...
      if (elements.adminPanelBtn) {
        elements.adminPanelBtn.style.display = 'inline-flex';
      }
      if (elements.mailboxSelector) {
        elements.mailboxSelector.style.display = 'block';
      }
      // 加载管理员的邮箱列表
...
async function loadAdminMailboxes() {
  try {
    // 管理员获取所有邮箱列表
    const response = await fetch('/api/mailboxes?limit=100');
    if (!response.ok) {
      throw new Error('加载邮箱列表失败');
    }
    const data = await response.json();
    userMailboxes = Array.isArray(data) ? data : [];

    // 更新邮箱选择器
    if (elements.mailboxSelector) {
      elements.mailboxSelector.innerHTML = '<option value="">请选择邮箱...</option>' +
        userMailboxes.map(m => {
          const addr = typeof m === 'string' ? m : (m.address || m);
          return `<option value="${addr}">${addr}</option>`;
        }).join('');

      // 如果只有一个邮箱，自动选中
      if (userMailboxes.length === 1) {
        const addr = typeof userMailboxes[0] === 'string' ? userMailboxes[0] : (userMailboxes[0].address || userMailboxes[0]);
        elements.mailboxSelector.value = addr;
        selectMailbox(addr);
      } else if (userMailboxes.length === 0) {
        // 没有邮箱时显示提示
        if (elements.currentMailbox) {
          elements.currentMailbox.textContent = '暂无邮箱，请先创建邮箱';
        }
      }
    }
  } catch (error) {
    console.error('加载邮箱列表失败:', error);
    showToast('加载邮箱列表失败', 'error');
  }
}
...
async function selectMailbox(address) {
  if (!address) {
    currentMailbox = null;
    if (elements.currentMailbox) {
      elements.currentMailbox.textContent = '';
    }
    emails = [];
    renderEmailList();
    return;
...
function bindEvents() {
  ...
  // 邮箱选择器（管理员专用）
  elements.mailboxSelector?.addEventListener('change', (e) => {
    selectMailbox(e.target.value);
  });
...
function updateRoleBadge() {
  if (elements.roleBadge && currentUser) {
    if (currentUser.role === 'admin') {
      elements.roleBadge.textContent = '管理员';
      elements.roleBadge.title = '管理员';
    } else {
