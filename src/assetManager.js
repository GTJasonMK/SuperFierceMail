...
export class AssetManager {
  ...
  async checkProtectedPathAuth(request, JWT_TOKEN, url) {
    ...
    // 根据路径和角色进行权限检查
    if (url.pathname.includes('mailbox')) {
      // 邮箱页面允许 admin 和 mailbox 角色访问
      if (payload.role !== 'mailbox' && payload.role !== 'admin') {
        return Response.redirect(new URL('/login.html', url).toString(), 302);
      }
    } else {
      // 其他受保护页面（如 admin.html）
      const isAllowed = (payload.role === 'admin' || payload.role === 'mailbox');
      if (!isAllowed) {
        // 已登录但权限不足：引导回登录页
        return Response.redirect(new URL('/login.html', url).toString(), 302);
      }
  ...
  async handleAdminPage(request, env, JWT_TOKEN) {
    ...
    if (!payload) {
      const loadingReq = new Request(
        new URL('/templates/loading.html?redirect=%2Fhtml%2Fadmin.html', url).toString(),
        request
      );
